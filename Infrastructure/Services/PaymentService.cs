...
public class PaymentService : IPaymentService
{
    ...
    {
        var cart = await cartService.GetCartAsync(cartId) ?? throw new Exception("Cart not available");
    ...
    private async Task<long> ApplyDiscountAsync(Core.Entities.Coupon coupon, long amount)
    {
        var couponService = new Stripe.CouponService();

        var stripeCoupon = await couponService.GetAsync(coupon.CouponId);
    ...
    private async Task ValidateCartItemsInCartAsync(ShoppingCart cart)
    {
        ...
        {
            var productItem = await unit.Repository<Core.Entities.Product>().GetByIdAsync(item.ProductId) ?? throw new Exception("Problem getting product in cart");
